...
<style>
    .t_nav span a {color:#8c8c8c;width:auto;}
</style>

<article class="blogs">
    <h1 class="t_nav">
    <span>
      <a>您当前的位置：</a><a href="{{url('/')}}">首页</a>
        @if ($field->cate_pid != 0)
            <a>&nbsp;&gt;&nbsp;</a><a href="{{url('cate/'.$field->cate_pid)}}">编程学习</a>
        @endif
        <a>&nbsp;&gt;&nbsp;</a><a href="{{url('cate/'.$field->cate_id)}}">{{$field->cate_name}}</a>
    </span>
       ...
       <div class="rnav">
          <ul>
              @foreach($submenu as $k=>$s)
                <li class="rnav{{$k+1}}"><a href="{{url('cate/'.$s->cate_id)}}">{{$s->cate_name}}</a></li>
              @endforeach
         </ul>
        </div>
    @endif
